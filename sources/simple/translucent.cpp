...
void update()
{
	...
	for (uint32 i = 0; i < knotsCount * 2; i++)
	{
		...
		e->value<ColorComponent>().opacity = smootherstep(clamp(p * 2, 0, 1));
	}
...
int main(int argc, char *args[])
{
	try
	{
		...
		for (uint32 i = 0; i < bulbsCount; i++)
		{
			...
			t.scale = 0.1;
			t.position = randomDirection3() * randomRange(1, 5) + Vec3(0, 3, 0);
			e->value<LightComponent>().lightType = LightTypeEnum::Point;
			e->value<ColorComponent>() = { colorHsvToRgb(randomChance3() * Vec3(1, .5, .5) + Vec3(0, .5, .5)), 20 };
		}
		for (uint32 j = 0; j < 3; j++)
		{
